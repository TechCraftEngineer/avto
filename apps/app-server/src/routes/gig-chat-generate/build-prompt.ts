...
export function buildGigPrompt(
  ...
  parts.push(
    "2. ШАГ 2 (стек технологий): для технических задач (лендинг, сайт, бот, приложение, скрипт, интеграция, дизайн в Figma и т.д.) — обязательно спроси «На каком стеке реализовать?» и заполни requiredSkills. Предложи варианты в quickReplies (например: «React/Next.js», «WordPress», «Telegram Bot API», «Python», «На усмотрение исполнителя»). Для нетехнических задач (тексты, переводы, монтаж видео) стек можно пропустить.",
  );
  parts.push(
    "3. ШАГ 3 (бюджет): если не указан — спроси «Какой бюджет планируете?» и предложи варианты в quickReplies (например: «до 5 000 ₽», «5 000 – 15 000 ₽», «15 000 – 50 000 ₽»).",
  );
  parts.push(
  ...
  parts.push(
    "7. quickReplies — 2–5 КОНКРЕТНЫХ вариантов как в quiz: пользователь может нажать кнопку ИЛИ написать свой вариант. Примеры: для «что нужно» — «Лендинг», «Telegram-бот»; для стека — «React/Next.js», «WordPress», «На усмотрение»; для бюджета — «до 5 000 ₽», «5 000 – 15 000 ₽»; для сроков — «Срочно (1-3 дня)», «Неделя», «2 недели». Каждый вариант = готовый ответ, который можно отправить одним кликом.",
  );
  parts.push("");
