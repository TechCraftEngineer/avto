...
export function ResponseSummaryCard({ response, gig }: SummaryCardProps) {
  ...
  const skillsMatchPercent = requiredSkills.length > 0 
    ? Math.round((matchedSkills.length / requiredSkills.length) * 100)
    : null;

  // Price comparison
  const price = response.proposedPrice;
  const priceStatus = price && gig && gig.budgetMin && gig.budgetMax ? (() => {
    const avgBudget = (gig.budgetMin + gig.budgetMax) / 2;
    const diff = ((price - avgBudget) / avgBudget) * 100;
    if (Math.abs(diff) < 10) return { label: "В рынке", color: "text-green-600", icon: CheckCircle2 };
    if (diff < 0) return { label: `Дешевле на ${Math.abs(Math.round(diff))}%`, color: "text-green-600", icon: TrendingDown };
    return { label: `Дороже на ${Math.round(diff)}%`, color: "text-orange-600", icon: TrendingUp };
  })() : null;
  ...
  const deliveryStatus = delivery && gig ? (() => {
    const diff = delivery - gigTimelineDays;
    if (diff <= 0) return { label: `Быстрее на ${Math.abs(diff)} дн.`, color: "text-green-600", icon: TrendingDown };
    if (diff <= 1) return { label: "Стандартные сроки", color: "text-green-600", icon: CheckCircle2 };
    return { label: `Медленнее на ${diff} дн.`, color: "text-orange-600", icon: TrendingUp };
  })() : null;

  // Overall score status
  const overallScore = screening?.overallScore;
  const scoreStatus = overallScore ? (() => {
    if (overallScore >= 80) return { label: "Отличное совпадение", color: "text-green-600", variant: "default" as const };
    if (overallScore >= 60) return { label: "Хорошее совпадение", color: "text-blue-600", variant: "secondary" as const };
    if (overallScore >= 40) return { label: "Среднее совпадение", color: "text-yellow-600", variant: "outline" as const };
    return { label: "Слабое совпадение", color: "text-red-600", variant: "outline" as const };
  })() : null;

  // Show only if there's meaningful data
  const hasData = skillsMatchPercent !== null || priceStatus || deliveryStatus || overallScore;
  ...
  return (
    <div className="bg-gradient-to-r from-primary/5 to-primary/10 border-primary/20 rounded-lg p-4 space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          ...
          <div className="flex items-center gap-2 p-2 rounded-md bg-background/50">
            ...
            <div className="min-w-0">
              ...
              <div className="font-medium truncate">
                {skillsMatchPercent}% ({matchedSkills.length}/{requiredSkills.length})
              </div>
            </div>
          </div>
        )}
        ...
        {priceStatus && (
          <div className="flex items-center gap-2 p-2 rounded-md bg-background/50">
            <priceStatus.icon className={`h-4 w-4 shrink-0 ${priceStatus.color}`} />
            <div className="min-w-0">
        ...
        {deliveryStatus && (
          <div className="flex items-center gap-2 p-2 rounded-md bg-background/50">
            <deliveryStatus.icon className={`h-4 w-4 shrink-0 ${deliveryStatus.color}`} />
            <div className="min-w-0">
        ...
        {overallScore && (
          <div className="flex items-center gap-2 p-2 rounded-md bg-background/50 sm:col-span-2 lg:col-span-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-muted-foreground text-xs">Детализация оценки</div>
              <div className="flex flex-wrap gap-2 mt-1">
                {screening?.priceScore !== undefined && screening.priceScore !== null && (
                  <Badge variant="outline" className="text-xs">
                    Цена: {screening.priceScore}/100
                  </Badge>
                )}
                {screening?.skillsMatchScore !== undefined && screening.skillsMatchScore !== null && (
                  <Badge variant="outline" className="text-xs">
                    Навыки: {screening.skillsMatchScore}/100
                  </Badge>
                )}
                {screening?.experienceScore !== undefined && screening.experienceScore !== null && (
                  <Badge variant="outline" className="text-xs">
                    Опыт: {screening.experienceScore}/100
                  </Badge>
                )}
                {screening?.deliveryScore !== undefined && screening.deliveryScore !== null && (
                  <Badge variant="outline" className="text-xs">
