...
import { AlertCircle, CheckCircle, Sparkles, Star, UserCheck } from "lucide-react";
 ...
 */
function calculateOverallScore(screening: ScreeningData): { score: number; recommendation: RecommendationLevel; summary: string[]; strengths: string[]; concerns: string[] } {
  const scores: number[] = [];
  ...
  if (screening.careerTrajectoryScore != null) {
    ...
    if (screening.careerTrajectoryType === "growth" || screening.careerTrajectoryType === "stable") {
  ...
  if (screening.hiddenFitIndicators && screening.hiddenFitIndicators.length > 0) {
 ...
 */
export const ScreeningResultsCard = memo(function ScreeningResultsCard({ screening, hasInterviewScoring = false }: ScreeningResultsCardProps) {
  ...
  const overallAssessment = useMemo(() => calculateOverallScore(screening), [screening]);
  ...
  const getInterviewQuestions = (): string[] | null => {
    if (screening.interviewQuestions && screening.interviewQuestions.length > 0) {
      return screening.interviewQuestions;
    }
    if (screening.psychometricAnalysis?.recommendations && screening.psychometricAnalysis.recommendations.length > 0) {
      return screening.psychometricAnalysis.recommendations;
    }
    return null;
  };
  const interviewQuestions = getInterviewQuestions();
  const hasInterviewQuestions = interviewQuestions !== null && interviewQuestions.length > 0;
      ...
      <CardContent className="space-y-4 sm:space-y-6">
        ...
        <div className="p-4 rounded-lg border-2 bg-gradient-to-r from-primary/5 to-primary/10">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <span className={cn("text-2xl font-bold", getScoreColor(getScoreTheme(overallAssessment.score)))}>
          ...
          <Progress 
            ...
            className="h-2 mb-3"
            indicatorClassName={getProgressColor(getScoreTheme(overallAssessment.score))}
          />
          
          <div className="flex items-center gap-2 mb-3">
            {(() => {
              const config = getRecommendationConfig(overallAssessment.recommendation);
          ...
          {screening.careerTrajectoryScore != null && screening.careerTrajectoryScore !== undefined && (
            <ScoreDisplay
              ...
              label={
                <span className="flex items-center gap-1">
                  Карьерная траектория
                  <InfoTooltip content={SCREENING_TOOLTIPS.careerTrajectory} />
                </span>
              }
              size="md"
            />
