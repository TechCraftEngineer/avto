// Shared response detail tabs and related components

// Re-export cards that are used in tabs
export { InterviewScoringCard } from "../response-detail-cards/interview-scoring-card";
export { ScreeningResultsCard } from "../response-detail-cards/screening-results-card";
export { ResumeActions } from "./actions/resume-actions";
export { VoicePlayer } from "./actions/voice-player";
export { ResponseHeader } from "./headers/response-header";
export { ComparisonTab } from "./tabs/comparison-tab";
export { ContactsTab } from "./tabs/contacts-tab";
export { DialogTab } from "./tabs/dialog-tab";
export { GigProposalTab } from "./tabs/gig-proposal-tab";
export { NotesTagsTab } from "./tabs/notes-tags-tab";
export { PortfolioTab } from "./tabs/portfolio-tab";
export { ProposalTab } from "./tabs/proposal-tab";
export { TimelineTab } from "./tabs/timeline-tab";
export { VacancyProposalTab } from "./tabs/vacancy-proposal-tab";
export { ValueChangeDisplay } from "./tabs/value-change-display";
