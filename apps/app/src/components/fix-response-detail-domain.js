...
/**
 * Исправление домена response-detail - перенос компонентов в components/
 */
...
const responseDetailPath = path.join(COMPONENTS_DIR, 'response-detail');

console.log('🔧 ИСПРАВЛЕНИЕ ДОМЕНА RESPONSE-DETAIL\n');
...
// Получаем все подпапки в response-detail
const subdirs = fs.readdirSync(responseDetailPath)
  .filter(item => fs.statSync(path.join(responseDetailPath, item)).isDirectory())
  .filter(dir => dir !== 'components' && dir !== 'hooks' && dir !== 'types' && dir !== 'utils'); // Исключаем служебные папки

let totalMoved = 0;

subdirs.forEach(subdir => {
  const subdirPath = path.join(responseDetailPath, subdir);
  const componentsPath = path.join(responseDetailPath, 'components');
...
// Обновляем главный index.ts для домена response-detail
const mainIndexPath = path.join(responseDetailPath, 'index.ts');
...
const finalContent = existingContent.includes('// response-detail domain exports')
  ? existingContent.replace(/\/\/ response-detail domain exports[\s\S]*/, `// response-detail domain exports\n${newExports}\n`)
...
console.log('✅ ДОМЕН RESPONSE-DETAIL ИСПРАВЛЕН!');