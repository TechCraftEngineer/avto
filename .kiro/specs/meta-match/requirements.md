...
Meta-Match — рекомендательный модуль оценки кандидата по трем уровням: Hard Skills, Soft Skills и Chronobiological Fit. Модуль не заменяет решения рекрутера, а добавляет дополнительный слой сигналов и аккуратно объясняет, как они интерпретируются в контексте найма.
