...
/**
 * Скрипт для исправления методов процедур в oRPC
 *
 * В oRPC нет отдельных методов .query() и .mutation()
 * Все процедуры используют .handler()
 */
...
function fixProcedureMethods(content: string): string {
  // Заменяем .query( на .handler(
  content = content.replace(/\.query\(/g, ".handler(");

  // Заменяем .mutation( на .handler(
  content = content.replace(/\.mutation\(/g, ".handler(");

  // Заменяем .subscription( на .handler(
  content = content.replace(/\.subscription\(/g, ".handler(");
...
async function processFile(filePath: string): Promise<boolean> {
  try {
    ...
    content = fixProcedureMethods(content);
