...
const CHANNEL_PROMPTS = {
  webChat: {
    ...
    description: "Сообщение, которое кандидат увидит при первом обращении в веб-чат",
    context: "Веб-чат на сайте - это первый контакт кандидата с компанией через онлайн-форму",
    guidelines: `
...
- Подчеркни готовность помочь и ответить на вопросы`,
  ...
  telegram: {
    ...
    description: "Сообщение, которое кандидат получит при начале диалога в Telegram",
    context: "Telegram бот - это продолжение общения с кандидатом через мессенджер",
    guidelines: `
