...
// РРјРїРѕСЂС‚РёСЂСѓРµРј С‚РѕР»СЊРєРѕ С‚Рµ РїСЂРѕС†РµРґСѓСЂС‹, РєРѕС‚РѕСЂС‹Рµ РЅРµ РёРјРµСЋС‚ РїСЂРѕР±Р»РµРјРЅС‹С… Р·Р°РІРёСЃРёРјРѕСЃС‚РµР№
import { list } from "./routers/workspace/list";
import { get } from "./routers/workspace/get";
import { create } from "./routers/workspace/create";
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (workspace) => {
          ...
          if (workspace.description !== null && workspace.description !== undefined) {
            expect(typeof result.description).toBe("string");
          } else {
            expect(result.description === null || result.description === undefined).toBe(
              true,
            );
          }
        },
