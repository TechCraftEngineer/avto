...
export const secureSchemas = {
  ...
  password: z.string()
    ...
    .regex(PATTERNS.STRONG_PASSWORD, 'Пароль должен содержать заглавные и строчные буквы, цифры и специальные символы'),

  // Username validation
  username: z.string()
    .min(3, 'Имя пользователя должно содержать минимум 3 символа')
    .max(30, 'Имя пользователя слишком длинное')
    .regex(PATTERNS.USERNAME, 'Имя пользователя может содержать только буквы, цифры, подчеркивания и дефисы'),

  // Phone validation
  phone: z.string()
    .regex(PATTERNS.PHONE, 'Некорректный формат телефона'),
  ...
  id: z.string()
    .regex(PATTERNS.UUID_V7, 'Некорректный формат ID'),
...
export const contentSecurity = {
  ...
  isMalicious: (content: string): boolean => {
    return PATTERNS.SQL_INJECTION.test(content) || 
  ...
  validateFileType: (filename: string, mimeType: string): boolean => {
    ...
    return allowedMimes.includes(mimeType) && 
           allowedExtensions[mimeType as keyof typeof allowedExtensions]?.includes(extension);
  },
};
