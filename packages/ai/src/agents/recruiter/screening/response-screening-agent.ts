...
2. ОСНОВНЫЕ ОЦЕНКИ:
   ...
   - СНАЧАЛА определи, относится ли опыт кандидата к той же профессиональной области, что и вакансия
   - Если кандидат из СОВЕРШЕННО ДРУГОЙ сферы (например, программист на вакансию няни, бухгалтер на вакансию водителя), ставь detailedScore 0-10
   - Даже если у кандидата есть "передаваемые навыки", это НЕ компенсирует полное отсутствие профильного опыта
   - Оценка 20+ возможна ТОЛЬКО если есть хотя бы минимальный релевантный опыт в нужной области
   
   a) detailedScore (0-100) - Детальная оценка соответствия резюме требованиям:
      - 0-10: ПОЛНОСТЬЮ нерелевантный опыт (другая профессиональная область, спам)
      - 11-20: Нерелевантный опыт с минимальными пересечениями
      - 21-40: Есть релевантный опыт, но критическое несоответствие требованиям
      - 41-60: Релевантный опыт, слабое/среднее соответствие требованиям
      - 61-80: Релевантный опыт, хорошее соответствие требованиям
      - 81-100: Релевантный опыт, отличное соответствие требованиям
...
3. ДЕТАЛЬНЫЕ ОЦЕНКИ (ОБЯЗАТЕЛЬНЫЕ ПОЛЯ, каждая от 0 до 100):
   ...
   - skillsMatchScore (ОБЯЗАТЕЛЬНО): Соответствие навыков требованиям
     * СНАЧАЛА проверь релевантность профессиональной области
     * Если опыт из другой сферы - ставь 0-15, даже если есть "передаваемые навыки"
   ...
   - experienceScore (ОБЯЗАТЕЛЬНО): Релевантность и глубина опыта
     * КРИТИЧЕСКИ ВАЖНО: Если весь опыт из другой профессиональной области - ставь 0-10
     ...
     * Опыт из смежных областей может давать 20-40, но ТОЛЬКО если области действительно смежные
...
4. ВЫЯВЛЕНИЕ СКРЫТЫХ ПОДХОДЯЩИХ (hiddenFitIndicators):
   ВАЖНО: Ищи "скрытых подходящих" ТОЛЬКО среди кандидатов с релевантным опытом!
   
   Ищи неочевидные сигналы о том, что кандидат может быть сильнее, чем кажется по резюме:
   - Переходы между смежными ролями ВНУТРИ одной профессиональной области (Бэкенд → Фулстек, Разработчик → Технический руководитель)
   - Рост сложности проектов в карьере
   - Нестандартный опыт, который релевантен для роли
   - Признаки высокой мотивации (самообучение, проекты вне работы) В РЕЛЕВАНТНОЙ ОБЛАСТИ
   - Передаваемые навыки из СМЕЖНЫХ областей (не из совершенно других сфер)
   
   НЕ СЧИТАЙ "скрытым подходящим" кандидата из другой профессиональной области, даже если он мотивирован.
...
ВАЖНО:
- СНАЧАЛА проверь релевантность профессиональной области - это главный фильтр
- Кандидат из другой сферы получает 0-10 баллов, независимо от "передаваемых навыков"
- Фокусируйся на потенциале кандидата ТОЛЬКО если он из релевантной области
- Выявляй "скрытых подходящих" ТОЛЬКО среди кандидатов с релевантным опытом
