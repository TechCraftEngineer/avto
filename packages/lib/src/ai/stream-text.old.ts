...
export function streamText(
  ...
  try {
    ...
    // Используем Object.create для сохранения прототипа и методов
    const wrappedResult = Object.create(Object.getPrototypeOf(result));
    Object.assign(wrappedResult, result);
    wrappedResult.textStream = callerStream;

    return wrappedResult as ReturnType<typeof aiStreamText>;
  } catch (error) {
    ...
    if (availableFallbacks.length > 0) {
      ...
      try {
        ...
        // Возвращаем fallback результат with untouched stream for caller
        // Используем Object.create для сохранения прототипа и методов
        const wrappedFallbackResult = Object.create(
          Object.getPrototypeOf(fallbackResult),
        );
        Object.assign(wrappedFallbackResult, fallbackResult);
        wrappedFallbackResult.textStream = fallbackCallerStream;

        return wrappedFallbackResult as ReturnType<typeof aiStreamText>;
      } catch (fallbackError) {
        fallbackGeneration?.end({
          statusMessage:
